...
class UE(object):
    def __init__(self, id, N_r=1, N_t=16, is_s=0, is_c=0):
        self.id = id
        self.N_r = N_r
        self.N_t = N_t
        # 
...
class BS(object):
    ...
    def _cal_noma(self, update = True):
        ...
        for _id in range(len(self.c_UE)):
            ...
            P_si = UE.g*UE.h_norm*UE.P_c
            P_in = 0.0
            if len(UE_in) > 0:
                P_in = UE.g*UE.h_norm*sum([UE_.P_c for UE_ in UE_in])
    ...
    def _cal_radar(self, update=True):   
        ...
        for _id in range(len(self.s_UE)):
            ...
            R_est.append(R_)
            print("RSINR(dB):",10*np.log(P_useful/(self.P_n*np.linalg.norm(UE.u)+P_in)) )
            print("Pin: ",P_in)
